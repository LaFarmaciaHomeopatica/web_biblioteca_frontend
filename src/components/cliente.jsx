...
// Proxy para ver PDFs (mismo enfoque que admin)
...
const Cliente = () => {
  ...
  // Cache global de productos-all (igual que admin)
  const productosAllCacheRef = useRef(null);

  // ===== Paginación en cliente (igual que admin) =====
  ...
  // ===== Fetch productos (MISMA LÓGICA ADMIN: siempre /productos-all con cache) =====
  const fetchProductos = useCallback(
    async (page = 1) => {
      ...
      const applyFiltersAndPaginate = (all) => {
        ...
        if (filterBy === "david") {
          const filtered = term
            ? (all || []).filter((p) =>
                String(p?.david ?? "").toLowerCase().includes(term)
              )
        ...
        // filtros normales
        const filterToUse = validTextFilters.includes(filterBy)
          ? filterBy
          : "nombre";
      ...
      try {
        ...
        // Si es 401, cerramos sesión y llevamos al login
        ...
        console.error(
          "Error cargando productos (cliente):",
    ...
    [searchTerm, filterBy, paginarCliente, navigate, logout]
  );
  ...
  // ===== Fetch documentos (alineado a ADMIN: producto_codigo) =====
  const fetchDocumentosProducto = useCallback(async (producto) => {
    if (!producto?.codigo) {
    ...
    try {
      // Admin usa: params { producto_codigo }
  ...
  // ===== Fetch productos asociados (mismo enfoque admin) =====
  const fetchProductosAsociados = useCallback(async (producto) => {
    ...
    try {
      ...
      const ordered = codigos
        .map((c) => mapByCode.get(String(c).trim()))
        .filter(Boolean);

      setRelatedProducts(ordered);
    } catch (err) {
      console.error(
        "Error cargando productos asociados:",
        err?.response?.data || err?.message || err
      );
  ...
  const handleOpenDocumento = (doc) => {
    const rawPath =
      doc?.ruta || doc?.path || doc?.archivo || doc?.file_path || doc?.storage_path;
  ...
  const handleShowDetails = (producto) => {
    ...
    fetchDocumentosProducto(producto);
                        ...
                        <Card.Body className="p-3">
                          ...
                          >
                            ...
                            <p>
                              ...
                              {selectedProducto.estado_producto}
                            </p>
                            <p>
                              <strong>Precio Público con IVA:</strong>{" "}
                              {mostrarPrecio(selectedProducto.precio_publico)}
                            </p>
                            <p>
                              <strong>Precio Médico con IVA:</strong>{" "}
                              {mostrarPrecio(selectedProducto.precio_medico)}
                            </p>
                            <p>
                              <strong>IVA:</strong>{" "}
                              {formatearIVA(selectedProducto.iva)}
                            </p>
                            <p>
                              <strong>Requiere Fórmula Médica:</strong>{" "}
                              {selectedProducto.formula_medica}
                            </p>
                            <p>
                            ...
                            <p>
                              ...
                              {selectedProducto.estado_registro}
                            </p>
                            <p>
                            ...
                            <p>
                              ...
                              {selectedProducto.registro_sanitario}
                            </p>
                            <p className="mb-0">
                                    ...
                                    >
                                      ...
                                      >
                                        ...
                                        <div className="text-muted">
                                          Público:{" "}
                                          {mostrarPrecio(p.precio_publico)} ·
                                          Médico:{" "}
                                          {mostrarPrecio(p.precio_medico)}
                                        </div>
                                      </div>
